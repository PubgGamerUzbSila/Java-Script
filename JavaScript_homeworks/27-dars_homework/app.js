//  Buni ustoz video darsda ko'rganman
 let  lets = {
...
(function(button) {
   lets[button].addEventListener("click", function() {
    calculate(button);
  });
...
if (str.length === 1) {
 ...
 let max = {
add: function() {
  if (! lets.displayInd.classList.toggle("max")) {
     lets.displayInd.classList.toggle("max");
  }
},
remove: function() {
  if ( lets.displayInd.classList.toggle("max")) {
...
function calculate(btn, isFromKey) {
 let operators = ["minus", "plus", "div", "mult"];

if (btn === "ac") {
   lets.display.innerText = "0";
   lets.displayInfo.innerText = "0";
  max.remove();
  return;
}

if (btn === "ce") {
   let result = removeLast( lets.displayInfo.innerText);
  if (!result) {
     lets.displayInfo.innerText = "0";
  } else {
     lets.displayInfo.innerText = result;
  }
  max.remove();
}

if (btn === "eq") {
   lets.displayInfo.innerText =  lets.display.innerText;
  max.remove();
  return;
}

 let operations =  lets.displayInfo.innerText;
...
operations = fixOper(operations);
 lets.displayInfo.innerText = operations;

 let calc = "" + eval( lets.display.innerText);
...
if (calc.length < 14) {
  if (!calc) {
     ...
     lets.display.innerText = calc;
  }

} else if (calc < 1) {
   lets.display.innerText = (+calc).toPrecision(7);
} else {
   lets.display.innerText = (+calc).toPrecision(9);
}
